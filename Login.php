...
if(isset($_POST['submit'])){
    ...
    if($lemail==$admin){
        ...
        oci_bind_by_name($result,":email",$Vemail);
        oci_bind_by_name($result,":password",$password);
        oci_bind_by_name($result,":type","user");
        oci_execute($result);
        $user=oci_fetch_array($result, OCI_ASSOC);
        oci_close($conn);
        if($user){
...
>>>>>>> Stashed changes
          ...
          <div class="mb-6">
            <label for="email" class="block mb-2 text-sm font-medium text-gray-900 ">Email address</label>
            <input type="text" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="john.doe@company.com" required>
          </div>
          <div>
            <label for="password" class="block mb-2 text-sm font-medium text-gray-900 ">Password</label>
            <input type="password" name="password" id="password" placeholder="••••••••" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " required="">
